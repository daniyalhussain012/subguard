...
import { getDaysUntil } from '../utils/storage'
...
// 5-tab bottom bar: Home, Radar, [Add+], Scan, More
const BOTTOM_TABS = [
  { to: '/', label: 'Home', icon: Home, end: true },
  { to: '/radar', label: 'Radar', icon: Radar },
  // center Add button is rendered separately
  { to: '/scanner', label: 'Scan', icon: Mail },
]

// Items shown in the "More" slide-up sheet
const MORE_ITEMS = [
  { to: '/subscriptions', label: 'All Subscriptions', icon: CreditCard },
  { to: '/household', label: 'Household Hub', icon: Users2 },
  { to: '/leaks', label: 'Money Leaks', icon: TrendingDown },
      ...
      <div className={`p-3 border-t space-y-1.5 ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
        {settings?.profile?.name && (
          <div className="flex items-center gap-2 px-2 py-1.5 mb-1">
            <span className="text-lg">{settings?.profile?.avatar || '👤'}</span>
            <div>
              <div className={`text-xs font-semibold ${darkMode ? 'text-slate-300' : 'text-slate-700'}`}>{settings.profile.name}</div>
              <div className="text-[10px] text-slate-500">Household admin</div>
            </div>
          </div>
        )}
        <button onClick={() => { navigate('/add'); onClose?.() }} className="btn-primary w-full justify-center text-xs py-2">
  ...
  return (
    <div className={`flex h-screen overflow-hidden ${darkMode ? 'bg-slate-950 text-slate-100' : 'bg-slate-100 text-slate-900'}`}>
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        ...
        <nav
          className={`lg:hidden fixed bottom-0 left-0 right-0 border-t z-30 ${darkMode ? 'bg-[#0F172A]/95 border-slate-800' : 'bg-white/95 border-slate-200'} backdrop-blur-md`}
        ...
        >
          <div className="flex items-center justify-around px-2">
            ...
            <NavLink
              ...
              className={({ isActive }) =>
                `flex flex-col items-center justify-center gap-0.5 py-2 px-3 rounded-xl text-[10px] font-semibold transition-colors min-w-[44px] min-h-[44px] ${
                  isActive ? 'text-cyan-400' : darkMode ? 'text-slate-500' : 'text-slate-400'
                }`
              }
            >
              <Home size={20} />
            ...
            >
              <div className="w-14 h-14 rounded-full bg-gradient-to-br from-cyan-400 to-cyan-600 flex items-center justify-center shadow-lg shadow-cyan-500/40 active:scale-95 transition-transform">
            ...
            <button
              ...
              className={`flex flex-col items-center justify-center gap-0.5 py-2 px-3 rounded-xl text-[10px] font-semibold transition-colors min-w-[44px] min-h-[44px] ${darkMode ? 'text-slate-500' : 'text-slate-400'}`}
            >
            ...
            <>
              ...
              >
                ...
                <div className="flex items-center justify-between px-5 py-3">
                  ...
                  <button onClick={() => setMoreOpen(false)} className="btn-icon">
                    <X size={18} />
                  </button>
                </div>
                ...
                {/* Profile + dark mode */}
                <div className={`mx-4 mb-4 p-3 rounded-xl flex items-center justify-between ${darkMode ? 'bg-slate-800' : 'bg-slate-100'}`}>
                  {settings?.profile?.name ? (
                    <div className="flex items-center gap-2">
                      <span className="text-xl">{settings?.profile?.avatar || '👤'}</span>
                      <div>
                        <div className={`text-sm font-semibold ${darkMode ? 'text-slate-200' : 'text-slate-800'}`}>{settings.profile.name}</div>
                        <div className="text-xs text-slate-500">Household admin</div>
                      </div>
                    </div>
                  ) : (
                    <span className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>SubGuard</span>
                  )}
                  <button
                    onClick={() => setDarkMode(!darkMode)}
                    className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs font-medium transition-colors ${darkMode ? 'bg-slate-700 text-slate-300' : 'bg-white text-slate-600'}`}
                  >
                    {darkMode ? <Sun size={14} /> : <Moon size={14} />}
                    {darkMode ? 'Light' : 'Dark'}
                  </button>
