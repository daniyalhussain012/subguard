...
Potential monthly savings: ${formatMonthlyByCurrency(canLiveWithout)}
Already saved (paused/cancelled): ${formatMonthlyByCurrency([...paused, ...cancelled])}/mo
    ...
    <div className="space-y-8">
      ...
      <div ref={reportRef} className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          {[
            { label: 'Saved Monthly', value: formatMonthlyByCurrency([...paused, ...cancelled]), sub: 'paused & cancelled', color: 'text-emerald-400' },
            { label: 'Potential Savings', value: formatMonthlyByCurrency(canLiveWithout), sub: 'cancel non-essentials', color: 'text-orange-400' },
            { label: 'Forgotten Charges', value: forgotten.length, sub: 'not reviewed 30d+', color: 'text-red-400' },
            { label: 'Small Leaks', value: formatMonthlyByCurrency(smallCharges), sub: `${smallCharges.length} charges under $5`, color: 'text-amber-400' },
          ...
          <div className="card border-cyan-500/25 p-5 flex items-center gap-4">
            ...
            <div>
              <p className="font-bold text-cyan-300 text-lg">
                Cut {canLiveWithout.length} non-essential subscription{canLiveWithout.length !== 1 ? 's' : ''} — save {formatMonthlyByCurrency(canLiveWithout)}/month
              </p>
              <p className="text-sm text-slate-400 mt-0.5">That's <span className="font-bold text-cyan-300">{formatMonthlyByCurrency(canLiveWithout, 12)}/year</span> back in your pocket</p>
            </div>
          </div>
        )}
          ...
          <div>
            ...
            <div className="card p-4 space-y-2">
              ...
              <div className="border-t border-slate-700/40 pt-2 flex justify-between">
                ...
                <span className="text-xs font-bold text-amber-400">{formatMonthlyByCurrency(smallCharges)}/mo · {formatMonthlyByCurrency(smallCharges, 12)}/yr</span>
              </div>
            </div>
          </div>
        )}
