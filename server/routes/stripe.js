...
const createSession = async (email, userId, frontendUrl) => {
  return getStripe().checkout.sessions.create({
    ...
    line_items: [{ price_data: { currency: 'usd', product_data: { name: 'SubGuard Pro', description: 'Unlimited subscriptions, all features. One-time payment.' }, unit_amount: 500 }, quantity: 1 }],
    mode: 'payment',
  ...
  try {
    ...
    if (session.payment_status === 'paid') {
      const user = await User.findByIdAndUpdate(
        ...
        { plan: 'premium', stripeSessionId: session.id, premiumActivatedAt: new Date() },
        { new: true }
      );
